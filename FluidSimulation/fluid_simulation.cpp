...
void FluidSimulation::Get_Full_Velocity() {
    for (int i = 0; i < GRIDX; i++) {
        for (int j = 0; j < GRIDY; j++) {
            for (int k = 0; k < GRIDZ; k++) {
                if (cubic.mask(i, j, k) != WATER && i + 1 < GRIDX && cubic.mask(i + 1, j, k) != WATER) {
                    Float x = nearest[i][j][k]->x, y = nearest[i][j][k]->y, z = nearest[i][j][k]->z;
                ...
                if (cubic.mask(i, j, k) != WATER && j + 1 < GRIDY && cubic.mask(i, j + 1, k) != WATER) {
                    Float x = nearest[i][j][k]->x, y = nearest[i][j][k]->y, z = nearest[i][j][k]->z;
                ...
                if (cubic.mask(i, j, k) != WATER && k + 1 < GRIDZ && cubic.mask(i, j, k + 1) != WATER) {
                    Float x = nearest[i][j][k]->x, y = nearest[i][j][k]->y, z = nearest[i][j][k]->z;
